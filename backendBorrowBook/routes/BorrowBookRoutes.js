...
router.post('/borrowbookregister', async (req, res) => {
  try {
    ...
    if (existingUser && existingBook) {
      ...
      const existingBorrow = await BorrowBook.findOne({ id: req.body.id });
      
      if (existingBorrow) {
        return res.status(409).json({ message: 'User already borrowed a book.' });
      }
      ...
      return res.status(409).send({ message: 'User or Book is not valid' });
    }
  } catch (error) {
...
router.get('/bookavailability/:bookid', async (req, res) => {
  try {
    ...
    if (existingBorrow) {
      return res.status(200).json({ available: false, message: 'Book is not available for borrowing.' });
    }

    // If the book exists and is not borrowed, it is available
    return res.status(200).json({ available: true, message: 'Book is available for borrowing.' });
  } catch (error) {
...
router.delete('/borrowbookregister/:_id', async (req, res) => {
  try {
    const deletedBorrowBook = await BorrowBook.findByIdAndDelete(req.params._id);
