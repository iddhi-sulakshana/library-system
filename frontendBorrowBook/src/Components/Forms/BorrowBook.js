...
const BorrowBook = () => {
   ...
   const [formData, setBookData] = useState({
      ...
      const handleChange = async(e) => {
        ...
        if (name === 'id' && value.trim() !== '') {
          try {
            const response = await axios.get(`http://localhost:5000/users/${value}`);
        ...
        if (name === 'bookid' && value.trim() !== '') {
          try {
            const response = await axios.get(`http://localhost:5000/borrowbook/bookavailability/${value}`);
            const isBookAvailable = response.data.available;
    
            if (!isBookAvailable) {
              setAvailabilityMessage('This book is not available for borrowing.');
            } else {
      ...
      const handleSubmit = (e) => {
        ...
        if (isUpdateMode) {
          ...
          axios
            .put(`http://localhost:5000/borrowbook/update/${formData._id}`, formData)
            .then((response) => {
          ...
          axios.post('http://localhost:5000/borrowbook/borrowbookregister', 
              formData,
  ...
  return (
    <div className="container mt-5">
       <h2 className="mb-4">{isUpdateMode ? 'Update Book' : 'Book Registration'}</h2>
      <form onSubmit={handleSubmit} >
        ...
        <div className="mb-3">
          ...
          <input type="number" className="form-control" name="pno" value={formData.pno || ''} onChange={handleChange} required />
        </div>
        <div className="mb-3">
          <label  className="form-label">Tack Date:</label>
          <input type="date" className="form-control" name="tackdate" value={formData.tackdate || ''} onChange={handleChange} required />
        </div>
        <div className="mb-3">
