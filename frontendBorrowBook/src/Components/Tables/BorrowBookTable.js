...
const BorrowBookTable = () => {
  ...
  const handleDelete = (_id) => {
    axios
      .delete(`http://localhost:5000/borrowbook/borrowbookregister/${_id}`)
      .then((response) => {
        // Update the state after successful deletion
        const updatedData = BorrowBookData.filter((row) => row._id !== _id);
  ...
  const calculateFine = (deliveryDate) => {
    ...
    if (formattedDeliveryDate < today) {
      const daysLate = Math.ceil((today - formattedDeliveryDate) / (1000 * 60 * 60 * 24));
    ...
    <table className="table table-responsive">
      ...
      {BorrowBookData.map((row,index) => (
           <tr key={index}>
              ...
              <td>
                <button type="button" class="btn btn-outline-danger" onClick={() => handleDelete(row._id)} >Return Book</button>
